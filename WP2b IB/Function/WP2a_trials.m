...
what.axisControl = NaN(ntrials,1);
...
mat0 = repmat(X0,[((1/4)*NumberOfBloc),1]); %The matrix for valid trials

catch1 = ones(36*NumberOfBloc,1); %In valid trials catcht=1
mat0(:, 6) = catch1;

% We need a new matrix for catch trials. We will keep the number of this to 10% of the trials
% Since this will not be analyzed, all parameters are random & not counterbalanced

mat1 = zeros(4*NumberOfBloc,6); %In catch trials catcht=0
...
mat = [mat0;mat1]; %The matrix for all trials
fluency =  mat(:, 1);
matrix = mat(:, 2);
noise = mat(:, 3);
question2 = mat(:, 4);
...
gains3 = gains1(1:4*NumberOfBloc, 1:1); % Gains for catch trials, to complete the number of trials
gains2 = [onegain2; symgain2; asymgain2];
gains2 = repmat(gains2, [4*NumberOfBloc, 1]);
gains4 = gains2(1:4*NumberOfBloc, 1:1); % Gains for catch trials, to complete the number of trials
          
gains1 = [gains1; gains3];
gains2 = [gains2; gains4];
             
% FULL MATRIX

X = table(fluency, matrix, noise, question2, delays, catcht, gains1, gains2); % table format in the basic order
...
for ibloc=1:NumberOfBloc
    ...
    p1 = [p1 p(ibloc).mesgain+(40*(ibloc-1))];
    if rem(ibloc,2) == 0
...
position1 = repmat(position1, [3*NumberOfBloc,1]); % 3*4*24 --> 288 trials
position3 = position1(1:4*NumberOfBloc, 1:1);
position1 = [position1;position3];

%position of gain 2 (matrix 2 & matrix 3) % decisional noise
position2 = [2;3;4;1;3;4;1;2;4;1;2;3];
position2 = repmat(position2, [3*NumberOfBloc,1]); % 12*24 --> 288 trials
position4 = position2(1:4*NumberOfBloc, 1:1);
position2 = [position2; position4];
...
what.randomDelay = timing([randX.delays])*10^-3; 
