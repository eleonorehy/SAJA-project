...
addpath('/Users/agencyteam/Desktop/SLB IJN/WP1a intentional binding/Programme Experience/Function');
...
ResultFolderWe = '/Users/agencyteam/Desktop/SLB IJN/WP1a intentional binding/Stockage Resultat';
OutputFileMainWe = [ResultFolderWe 'Binding' sub '.mat'];
...
try
    ...
    trials  = WP1a_IB_trials ; % function that conterbalances all the experimental conditions
...
%      LoRiSquare = 0;
    ...
    OutputFileTrial = [ResultFolderWe 'IB' sub '.mat'];
...
if trials.blocRating(itrial) == 1 
    delay = 0.5
elseif trials.blocRating(itrial) == 2 
...
%         bloctriggers(itrial).nbloc = [bloctriggers(itrial).nbloc blocknumb]; 
         if trials.blocRating(itrial) == 1
              DrawFormattedText(expWindow, ['Durant ce block, vous devrez noter votre sentiment de contrôle de la bille'], 'center', 'center', white);            
              Screen('Flip', expWindow);
              WaitSecs(10);
         elseif trials.blocRating(itrial) == 2
             DrawFormattedText(expWindow, ['Durant ce block, vous devrez estimer l intervalle de temps entre les 2 beeps sonores'], 'center', 'center', white);            
             Screen('Flip', expWindow);
             WaitSecs(10);
         end
...
%%% ongoing TRIALS PARAMETERS
        ...
        Contrib(itrial).Xaxe = [];
        Contrib(itrial).Yaxe = [];
        t_spacePress(itrial).S1 = [];
        t_spacePress(itrial).S2 = [];
...
%         Screen('FrameRect', expWindow, frameColor, whichIsReached, penWidthPixels);
        ...
        if trials.blocRating(itrial) == 2
            
%             DrawFormattedText(expWindow, '...', 'center','center', white);
%             Screen('Flip', expWindow); 
%             WaitSecs(trials.randomDelay(itrial));
            if results.Gain(itrial) == 1;
                Beeper(500, 1, 0.150);
            elseif results.Gain(itrial) == 0;
                Beeper(350, 1, 0.150);
            end 
            DrawFormattedText(expWindow, '...', 'center','center', white);
            Screen('TextSize', expWindow, 80);
            ...
            DrawFormattedText(expWindow, 'Reproduisez le delai entre les 2 beeps en maintenant appuyée la barre espace', 'center','center', white);
            Screen('TextSize', expWindow, 30);
            Screen('Flip', expWindow);
            TstartDelay = GetSecs; 
%             WaitSecs(5);
            ...
            while DelayEstim == false;
                                                           
%             [keyIsDown1Sujet1,secs2Sujet1, keyCode2Sujet1] = PsychHID('KbCheck', keyboardIndices(1,1));
%             [keyIsDown1Sujet2,secs2Sujet2, keyCode2Sujet2] = PsychHID('KbCheck', keyboardIndices(1,2));
            ...
            if keyCodeSujet1(Key.escape) || keyCodeSujet2(Key.escape)
                ...
                if keyCodeSujet1(Key.space)
                    t_spaceKP_S1 = t_spaceKP_S1 + 1;
                    S1Estim = 1;
                end
                if keyCodeSujet2(Key.space)
                    t_spaceKP_S2 = t_spaceKP_S2 + 1;
                    S2Estim =  1;

                end
            
        t_spacePress(itrial).S1 = [t_spacePress(itrial).S1 t_spaceKP_S1];
        t_spacePress(itrial).S2 = [t_spacePress(itrial).S2 t_spaceKP_S2];
...
%                 if keyCode1Sujet2(Key.space)
%                     S2Estim = 1; 
%                 end 
%             end
            
            if GetSecs - TstartDelay > TendDelay
                    DelayEstim = true;
             end 
...
%         
   if trials.blocRating(itrial) == 1
        ...
        Q_1 = 'A quel point penses tu\n \ncontroler le mouvement du point (touches jaunes)';
...
%             rep_max2 = 'Controle\n \npartage';
%             place = 40;
            Q_2 = 'A quel point pensez vous\n \ncontroler ensemble le mouvement du point (touches vertes) ';
...
%         trialtriggers(itrial).nbloc = [trialtriggers(itrial).nbloc blocknumb];
        ...
        if   itrial == 24 | itrial == 48 | itrial == 72 | itrial == 96 | itrial == 120 | itrial == 144  % | itrial == 224 | itrial == 256 
...
%         bloctriggers(itrial).nbloc = [bloctriggers(itrial).nbloc blocknumb];
            ...
            Screen('TextSize', expWindow, 30);
            DrawFormattedText(expWindow, ['Block ' num2str(blocknumb) '/6 accompli \n\nPrenez une pause \n\nAppuyez sur espace pour reprendre \n\n'], 'center', 'center', white);            
            Screen('Flip', expWindow);
            passKey(Key.space);
...
%     bloctriggers(itrial).nbloc = [bloctriggers(itrial).nbloc blocknumb];
    ...
    Screen('DrawLine', expWindow, white, Info.xC, 0, Info.xC, Info.rect(4),2);
    Screen('TextSize', expWindow, 30);   
    DrawFormattedText(expWindow, ['Merci de votre participation \n \n Player1 a gagne :' '\n \n' num2str(results.TotalGain) ' euros'], 'center', 'center', white, [], [], [], [], [],[Info.rect(1) Info.rect(2) Info.xC, Info.rect(4)]);
    DrawFormattedText(expWindow, ['Merci de votre participation \n \n Player2 a gagne :' '\n \n' num2str(results.TotalGain) ' euros'], 'center', 'center', white, [], [], [], [], [], [Info.xC Info.rect(2) Info.rect(3), Info.rect(4)]);
    Screen('Flip', expWindow);
    passKey(Key.space);
...
%     bloctriggers(itrial).nbloc = [bloctriggers(itrial).nbloc blocknumb];
    ...
    save(OutputFileMainWe, 'condition', 'which', 'sub', 'gender', 'date', 'trials', 'results', 'data', 'Contrib', 't_spacePress');
