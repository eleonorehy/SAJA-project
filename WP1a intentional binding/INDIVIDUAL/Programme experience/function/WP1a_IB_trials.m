...
what.axisControl = NaN(96,1);

PASDENOM = [1 25 49 73 97] % 225 257 289];

p1 = [];

X0 = fullfact([2,4,1]); % matrice compl?te : bruit / position du gain / type d'essai (only regular for this matrix)
X1 = fullfact([1,4,3]); %matrice of catch trials : type of fluency does not matter for this matrix
...
x2 = repmat([-1 1]', [48,1]); %repmat([-1 1]', [96,1]);
...
B0 = repmat([1:2]', [2,1]); %B0 = repmat([1:3]', [3,1])
B1 = randperm(size(B0, 1));
B2 = B0(B1);

nblocs = 4 ; % to suppress after 3 blocs pilote
