...
addpath('/Users/agencyteam/Desktop/SLB IJN/WP1a intentional binding/Programme Experience/Training/Function');
PsychDefaultSetup(2);
KbName('UnifyKeyNames');

% TRIGGERS --> OPENING SERIAL PORT
...
% key.leftUp = KbName('KA'); key.rightUp = KbName('LA');
% key.leftDown = KbName('KZ'); key.rightDown = KbName('LZ');
...
%triggers = []; % matrice for triggers informations
...
ResultFolderSelf = '/Users/agencyteam/Desktop/SLB IJN/EDA pilote1/data/';
OutputFileMainSelf = [ResultFolderSelf 'EDA1' sub '.mat'];
...
%     bloctriggers(itrial).cond = [];
    
    
   if itrial == 1 %|itrial == 25 | itrial == 49 | itrial == 73 | itrial == 97 | itrial == 121 | itrial == 145 | itrial == 169 | itrial == 193 | itrial == 217 | itrial == 241 | itrial == 265 

      
%       if itrial == 1                    
%         fprintf(s1,['mh',0011]) % DECIMAL 3, BIT 7 --> comment7
%         marker = marker + 1;
%         bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%         bloctriggers(itrial).stim = [bloctriggers(itrial).stim 1]; %'start experiment'
%         bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%         bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
%         
%       else
%         fprintf(s1,['mh',0011]) % DECIMAL 3, BIT 7 --> comment7
%         marker = marker + 1;
%         bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%         bloctriggers(itrial).stim = [bloctriggers(itrial).stim 2]; %'start full bloc'
%         bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%         bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
%       end
...
%       marker = marker + 1;
%       bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%       bloctriggers(itrial).stim = [bloctriggers(itrial).stim 3]; %'start bloc baseline'
%       bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%       bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
%       
%       DrawFormattedText(expWindow, ['Relax...\n \n Bougez le moins possible pendant les 2 prochaines minutes '], 'center', 'center', white);            
%       Screen('Flip', expWindow);
%       WaitSecs(2);
%       
%       fprintf(s1,['mh',0011]) % DECIMAL 2, BIT 7 --> comment7
%       marker = marker + 1;
%       bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%       bloctriggers(itrial).stim = [bloctriggers(itrial).stim 4]; %'end bloc baseline'
%       bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%       bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
...
%       marker = marker + 1;
%       bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%       bloctriggers(itrial).stim = [bloctriggers(itrial).stim 5]; %'start instructions'
%       bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%       bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
...
%       marker = marker + 1;
%       bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%       bloctriggers(itrial).stim = [bloctriggers(itrial).stim 6]; %'end bloc instructions'
%       bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%       bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
...
%         if trials.bloctype(itrial) == 1 && trials.kind(itrial) == 1 %easy condition
%             lowNoise = 1.5;
%             highNoise = 1.5;
%         elseif trials.bloctype(itrial) == 2 && trials.kind(itrial) == 1 %medium condition
%             lowNoise = 4;
%             highNoise = 4;
%         elseif trials.bloctype(itrial) == 3 && trials.kind(itrial) == 1 %hard condition
%             lowNoise = 5;
%             highNoise = 5;
...
%         trialtriggers(itrial).cond = [];
%         trialtriggers(itrial).type = [];
...
%         fprintf(s1,['mh',0010]) % DECIMAL 2, BIT 8 --> comment8
%         marker = marker + 1 ;
%         trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%         trialtriggers(itrial).stim = [trialtriggers(itrial).stim 7];% squares onset
%         trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%         trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%         trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
...
%         marker = marker + 1 ;
%         trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%         trialtriggers(itrial).stim = [trialtriggers(itrial).stim 8];% ball onset
%         trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%         trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%         trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%         trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%             elseif keyCode(Key.leftDown)
%                 pixelsPerPressDot = 0;
%                 
%             elseif keyCode(Key.leftUp)
%                 pixelsPerPressDot = 0;
%                 
%             elseif keyCode(Key.rightDown)
%                 pixelsPerPressDot = 0;
%                 
%             elseif keyCode(Key.rightUp)
%                 pixelsPerPressDot = 0;
...
%             lsqkqlqkqwlwl&akwkwk&a
%             data(itrial).Right = [data(itrial).Right rightKP];

            % AVOIDING THE DOT TO GET Out of THE WINDOW
            if dot.Xpos < 10
                dot.Xpos = 10;
...
%                 marker = marker + 1 ;
%                 trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%                 trialtriggers(itrial).stim = [trialtriggers(itrial).stim 9]; % missed square
%                 trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%                 trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%                 trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%                 trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%                 marker = marker + 1 ;
%                 trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%                 trialtriggers(itrial).stim = [trialtriggers(itrial).stim 10]; % green square --> success
%                 trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%                 trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%                 trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%                 trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%                 marker = marker + 1 ;
%                 trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%                 trialtriggers(itrial).stim = [trialtriggers(itrial).stim 11]; % red square --> error
%                 trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%                 trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%                 trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%                 trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%         marker = marker + 1 ;
%         trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%         trialtriggers(itrial).stim = [trialtriggers(itrial).stim 12]; % start questions
%         trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%         trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%         trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%         trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%         marker = marker + 1 ;
%         trialtriggers(itrial).time = [trialtriggers(itrial).time GetSecs];
%         trialtriggers(itrial).stim = [trialtriggers(itrial).stim 13]; % end questions
%         trialtriggers(itrial).num = [trialtriggers(itrial).num marker];
%         trialtriggers(itrial).cond = [trialtriggers(itrial).cond trials.bloctype(itrial)];
%         trialtriggers(itrial).type = [trialtriggers(itrial).type trials.kind(itrial)];
%         trialtriggers(itrial).ntrial= [trialtriggers(itrial).ntrial itrial];
...
%%%% breaks and feedback            
      ...
      if itrial == numberOfTrials % | itrial == 48 | itrial == 72 | itrial == 96 | itrial == 120 | itrial == 144 | itrial == 168 | itrial == 192 | itrial == 216 | itrial == 240 | itrial == 264 | itrial == 288
...
%         marker = marker + 1;
%         bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%         bloctriggers(itrial).stim = [bloctriggers(itrial).stim 14]; %'start break'
%         bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%         bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
%         
%         DrawFormattedText(expWindow, ['Vous  avez le droit a une pause.\n \nAppuyer sur espace pour vos stats au block ' num2str(blocknumb) '/12'], 'center', 'center', white);            
%         Screen('Flip', expWindow);
%         passKey(Key.space);
        DrawFormattedText(expWindow, ['Vous gagnez ' num2str(succeedTrials) '/12 points pour ce block.\n \nAppuyer sur espace pour quitter'], 'center', 'center', white);            
        Screen('Flip', expWindow);
        passKey(Key.space);
...
%         marker = marker + 1 ;
%         bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%         bloctriggers(itrial).stim = [bloctriggers(itrial).stim 15]; %'end break'
%         bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%         bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
    ...
    cagnotte = cagnotte + (results.TotalGain * 0.017);   
    
%     fprintf(s1,['mh',0011]) % DECIMAL 3, BIT 7 --> comment7
%     marker = marker + 1 ;
%     bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
%     bloctriggers(itrial).stim = [bloctriggers(itrial).stim 16]; %'start end message'
%     bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
%     bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
...
%     
% %     fprintf(s1,['mh',0010]) % DECIMAL 2, BIT 8 --> comment8
% %     trialtriggers.time(itrial) = GetSecs
% %     trialtriggers.stim(itrial) = 'end end message'
% %     trialtriggers.num(itrial) = marker + 1
%     
...
% marker = marker + 1 ;
% bloctriggers(itrial).time = [bloctriggers(itrial).time GetSecs];
% bloctriggers(itrial).stim = [bloctriggers(itrial).stim 17]; %'end of experiment'
% bloctriggers(itrial).num = [bloctriggers(itrial).num marker];
% bloctriggers(itrial).cond = [bloctriggers(itrial).cond trials.bloctype(itrial)];
% 
% fclose(s1);
