...
% List the different keyboard (don't forget to test if exactly 2 differents keyboards are
% detected)
...
while exitDemo == false
    ...
    [keyIsDown1,secs1, keyCode1] = PsychHID('KbCheck', keyboardIndices(1,1));
    [keyIsDown2,secs2, keyCode2] = PsychHID('KbCheck', keyboardIndices(1,2));

    % Depending on the button press, either move ths position of the square
    % or exit the demo
    if keyCode1(escapeKey) || keyCode2(escapeKey)
        exitDemo = true;
    end
    if keyIsDown1 == 1
        if keyCode1(leftKey)
